...
class WorkFlowService {
	...
	//--------------------------2014-9-5增加回流程通过、驳回、会签、转办、中止、挂起等核心操作---------------------------------------------------
...
//		}
	...
	public List<ActivityImpl> findBackAvtivity(String taskId) throws Exception {
		...
		//判断是否为会签节点，会签任务节点不允许退回；当前不考虑会签情况
	...
	private ProcessDefinitionEntity findProcessDefinitionEntityByTaskId(String taskId) throws Exception {
		// 取得流程定义
		ProcessDefinitionEntity processDefinition = (ProcessDefinitionEntity) repositoryService.getDeployedProcessDefinition(this.findTaskById(taskId).getProcessDefinitionId());
	...
	//--------------------------2014-9-5增加流程通过、驳回、会签、转办、中止、挂起等核心操作----------------------------------------------------
	...
	def getTaskEntityByActivity ={activityEntity ->
