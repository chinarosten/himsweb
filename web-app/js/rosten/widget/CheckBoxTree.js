...
define(["dojo/_base/declare",
	...
	var _CheckBoxStoreModel =  declare("rosten.widget.CheckBoxStoreModel", ForestStoreModel , {
	    ...
	    _updateParentCheckbox: function(/*dojo.data.Item*/storeItem, /*Boolean*/ newState){
	        ...
	        kernel.forEach(parents, function(parentItem){
	            if (newState) { // new state = true (checked)
	                this.getChildren(parentItem, lang.hitch(this, function(siblings){
	                    var allChecked = true;
	                    kernel.some(siblings, function(sibling){
	    ...
	    validateData: function(/*dojo.data.Item*/storeItem, /*thisObject*/ scope){
	        ...
	        scope.getChildren(storeItem, lang.hitch(scope, function(children){
	            var allChecked = true;
	            var childState;
	            kernel.forEach(children, function(child){
	                if (this.mayHaveChildren(child)) {
    ...
    var _CheckBoxTreeNode = declare("rosten.widget._CheckBoxTreeNode", Tree._TreeNode, {
    ...
    var _CheckBoxTree = declare("rosten.widget.CheckBoxTree", Tree, {
	    ...
	    _onCheckboxChange: function(/*dojo.data.Item*/storeItem){
	        ...
	        // tags:
	        //		callback
	        var model = this.model, identity = model.getIdentity(storeItem), nodes = this._itemNodesMap[identity];
	        ...
	        if (nodes) {
	            kernel.forEach(nodes, function(node){
	...
	//        this.model.validateData(this.model.root, this.model);
	    ...
	    _createTreeNode: function(args){
	        ...
	        // description:
	        //		Create a new CheckboxTreeNode instance.
	        return new rosten.widget._CheckBoxTreeNode(args);
	    }
