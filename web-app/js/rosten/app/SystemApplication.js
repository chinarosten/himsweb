/**
 * @author rosten
 */
define(["dijit/registry"
        ,"rosten/kernel/_kernel"
        ,"rosten/kernel/behavior"
        ,"rosten/app/Application"
        ], function(registry,_kernel, behavior,Application) {

    page_quit_1 = function(flag) {
        window.close();
        if (flag && flag == true) {
            window.opener.refreshSystem();
        }
    };
    page_quit = function() {
        rosten.pagequit();
    };
    selectCompany = function() {
        var id = "sys_companyDialog";
        var initValue = [];
        var companyName = registry.byId("companyName");
        if (companyName.attr("value") != "") {
            initValue.push(companyName.attr("value"));
        }
        rosten.selectDialog("机构选择", id, rosten.webPath + "/system/userSelectCompany", false, initValue);
        rosten[id].callback = function(data) {
            dijit.byId("companyName").attr("value", data[0].name);
            dijit.byId("companyId").attr("value", data[0].id);
        };
    };
    selectModel = function(companyId) {
        var id = "sys_modelDialog";
        var initValue = [];
        var modelName = dijit.byId("modelName");
        if (modelName.attr("value") != "") {
            initValue.push(modelName.attr("value"));
        }
        rosten.selectDialog("模块选择", id, rosten.webPath + "/system/modelSelect?companyId=" + companyId, false, initValue);
        rosten[id].callback = function(data) {
            dijit.byId("modelName").attr("value", data[0].name);
            dijit.byId("modelId").attr("value", data[0].id);
        };
    };
    selectDepart = function(url) {
        var id = "sys_departDialog";

        if (rosten[id] && dijit.byId(id)) {
            rosten[id].open();
            rosten[id].refresh();
        } else {
            var args = {
                url : url,
                rootLabel : "部门层级",
                showCheckBox : false,
                folderClass : "departTree"
            };
            rosten[id] = new rosten.widget.PickTreeDialog(args);
            rosten[id].open();
        }
        rosten[id].callback = function(data) {
            var _data = "";
            var _data_1 = "";
            for (var k = 0; k < data.length; k++) {
                var item = data[k];
                if (_data == "") {
                    _data += item.name;
                    _data_1 += item.id;
                } else {
                    _data += "," + item.name;
                    _data_1 += "," + item.id;
                }

            }
            dijit.byId("allowdepartsName").attr("value", _data);
            dojo.byId("allowdepartsId").value = _data_1;
        };
    };
    selectUser = function(url) {
        var id = "sys_userDialog";

        if (rosten[id] && dijit.byId(id)) {
            rosten[id].open();
            rosten[id].refresh();
        } else {
            var args = {
                url : url,
                rootLabel : "用户层级",
                showCheckBox : true,
                folderClass : "departTree"
            };
            rosten[id] = new rosten.widget.PickTreeDialog(args);
            rosten[id].open();
        }
        rosten[id].callback = function(data) {
            var _data = "";
            var _data_1 = "";
            for (var k = 0; k < data.length; k++) {
                var item = data[k];
                if (item.type == "user") {
                    if (_data == "") {
                        _data += item.name;
                        _data_1 += item.id;
                    } else {
                        _data += "," + item.name;
                        _data_1 += "," + item.id;
                    }
                }
            }
            dijit.byId("allowusersName").attr("value", _data);
            dojo.byId("allowusersId").value = _data_1;
        };
    };
    selectGroup = function(url) {
        var id = "sys_groupDialog";
        var initValue = [];
        var allowgroupsName = dijit.byId("allowgroupsName");
        if (allowgroupsName.attr("value") != "") {
            initValue.push(allowgroupsName.attr("value"));
        }
        rosten.selectDialog("群组选择", id, url, true, initValue);
        rosten[id].callback = function(data) {
            var _data = "";
            var _data_1 = "";
            for (var k = 0; k < data.length; k++) {
                var item = data[k];
                if (_data == "") {
                    _data += item.name;
                    _data_1 += item.id;
                } else {
                    _data += "," + item.name;
                    _data_1 += "," + item.id;
                }
            }
            dijit.byId("allowgroupsName").attr("value", _data);
            dojo.byId("allowgroupsId").value = _data_1;
        };
    };
    selectRole = function(url) {
        var id = "sys_roleDialog";
        var initValue = [];
        var allowrolesName = dijit.byId("allowrolesName");
        if (allowrolesName.attr("value") != "") {
            initValue.push(allowrolesName.attr("value"));
        }
        rosten.selectDialog("角色选择", id, url, true, initValue);
        rosten[id].callback = function(data) {
            var _data = "";
            var _data_1 = "";
            for (var k = 0; k < data.length; k++) {
                var item = data[k];
                if (_data == "") {
                    _data += item.name;
                    _data_1 += item.id;
                } else {
                    _data += "," + item.name;
                    _data_1 += "," + item.id;
                }
            }
            dijit.byId("allowrolesName").attr("value", _data);
            dojo.byId("allowrolesId").value = _data_1;
        };
    };
    selectPermission = function(url) {
        var id = "sys_permissionDialog";
        var initValue = [];
        var allowpermissionsName = dijit.byId("allowpermissionsName");
        if (allowpermissionsName.attr("value") != "") {
            initValue.push(allowpermissionsName.attr("value"));
        }
        rosten.selectDialog("权限选择", id, url, true, initValue);
        rosten[id].callback = function(data) {
            var _data = "";
            var _data_1 = "";
            for (var k = 0; k < data.length; k++) {
                var item = data[k];
                if (_data == "") {
                    _data += item.name;
                    _data_1 += item.id;
                } else {
                    _data += "," + item.name;
                    _data_1 += "," + item.id;
                }
            }
            dijit.byId("allowpermissionsName").attr("value", _data);
            dojo.byId("allowpermissionsId").value = _data_1;
        };
    };
    selectResource = function(url) {
        var id = "sys_resourceDialog";

        if (rosten[id] && dijit.byId(id)) {
            rosten[id].open();
            rosten[id].refresh();
        } else {
            var args = {
                url : url,
                rootLabel : "资源层级",
                showCheckBox : true,
                folderClass : "departTree"
            };
            rosten[id] = new rosten.widget.PickTreeDialog(args);
            rosten[id].open();
        }
        rosten[id].callback = function(data) {
            var _data = "";
            var _data_1 = "";
            for (var k = 0; k < data.length; k++) {
                var item = data[k];
                if (item.type == "resource") {
                    if (_data == "") {
                        _data += item.name;
                        _data_1 += item.id;
                    } else {
                        _data += "," + item.name;
                        _data_1 += "," + item.id;
                    }
                }
            }
            dijit.byId("allowresourcesName").attr("value", _data);
            dojo.byId("allowresourcesId").value = _data_1;
        };
    };
});
